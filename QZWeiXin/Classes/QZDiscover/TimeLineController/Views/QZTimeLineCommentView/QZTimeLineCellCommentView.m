...
- (void)setupWithLikeItmesArray:(NSArray *)likeItemsArray commentItemsArray:(NSArray *)commentItemsArray
{
    ...
    if (self.commentLabelsArray.count) {
        [self.commentLabelsArray enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger idx, BOOL * stop) {
            ...
            label.hidden = YES;//重用时先隐藏所有label，然后根据评论显示label
        }];
    }
    
    if (!commentItemsArray.count && !likeItemsArray.count) {
        self.fixedWidth = @(0); // 如果没有评论或者点赞，设置commentview固定宽度为0 （设置了fixedWidth的控件将不再自动布局过程中调整宽度）
        self.fixedHeight = @(0); //如果没有评论或者点赞，设置commentview的固定高度为0 （设置了fixedHeight的控件将不再在自动布局过程中调整高度）
        return;
    } else {
        self.fixedWidth = nil; //取消固定宽度约束
        self.fixedHeight = nil; // 取消固定高度约束
    }
